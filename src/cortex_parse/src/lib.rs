...
impl<'a> Parser<'_> {
    ...
    fn resolve_include(&mut self, file_path: FilePath) -> Result<Module> {
        let ctx = SessCtx::new(FileHandler::new(file_path).map_err(|e| Box::new(CortexError::from(e)))?);
    ...
    fn parse_term(&mut self) -> Result<Expr> {
        let node = match self.tok.kind.clone() {
            ...
            TokenKind::Id(_) => {
                if self.lexer.peek_token()?.kind == TokenKind::BraceOpen(BraceKind::Paren) {
                    let call = self.parse_call()?;
                    let call_span = call.ident().span().to(call.args_span());
                    return Ok(Expr::new(
                        ExprKind::Call(call),
                        self.new_loc(call_span),
                    ));
                } else {
    ...
    /// Parses a dot chain, e.g. `foo.bar`.
        ...
        loop {
            match self.tok.kind {
                TokenKind::Delim(DelimKind::DotSep) => {
                    self.advance()?;
                    span = span.to(&self.tok.span);
                    let member = self.parse_dot_member()?;
                    dot_res.add_member(member);
                },
                _ => break,
            }
        }
    ...
    fn parse_dot_member(&mut self) -> Result<MemberKind> {
        match self.tok.kind {
            TokenKind::Id(_) => {
                let member = match self.lexer.peek_token()?.kind {
                    TokenKind::BraceOpen(BraceKind::Paren) => MemberKind::Call(self.parse_call()?),
                    _ => MemberKind::Ident(self.parse_ident()?),
                };
                Ok(member)
            },
            _ => Err(CortexError::expected_but_got(&self.ctx, "identifier", &self.tok).into())
        }
    }
    ...
    fn parse_call(&mut self) -> Result<Call> {
        ...
        self.eat(TokenKind::BraceClosed(BraceKind::Paren))?;
        let args_span = open_paren_span.to(&close_paren_span);
        Ok(Call::new(ident, args, args_span))
    }
